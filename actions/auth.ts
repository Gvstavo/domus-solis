...
export async function registerUser(
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, salt);
     ...
     const insertQuery = "INSERT INTO usuarios(email, senha, nome, created_at) VALUES ($1, $2, $3,NOW()) RETURNING id";
...
export async function login(
  ...
  try {
  	...
  	const query = "SELECT email,senha,id,nome FROM usuarios WHERE email = $1";
  	const values = [email];
  	...
  	if(result.rowCount == 1){
  		if(await bcrypt.compare(senha, result.rows[0].senha)){
  			    await createSession({email: email,id:  result.rows[0].id, name:  result.rows[0].nome});
